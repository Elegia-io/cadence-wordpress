...
def main() -> int:
    ...
    # THE MINIMUM WORDPRESS, the same pair of files. .org shows readme.txt's
    # value while WordPress itself enforces the header's, so a bump of one
    # alone tells sites one minimum and refuses them at another.
    header_min = only_match(
        r"^\s*\*\s*Requires at least:\s*(\S+)\s*$", PLUGIN.read_text()[:8192],
        "Requires at least:", PLUGIN
    )
    readme_min = only_match(
        r"^Requires at least:\s*(\S+)\s*$", block, "Requires at least:", README
    )
    if header_min != readme_min:
        print(
            f"minimum drift: {PLUGIN.name} Requires at least: {header_min} "
            f"but {README.name} Requires at least: {readme_min}",
            file=sys.stderr,
        )
        return 1

    print(f"Version: {header} == Stable tag: {stable}")
    print(f"Requires at least: {header_min} in both")
    return 0
