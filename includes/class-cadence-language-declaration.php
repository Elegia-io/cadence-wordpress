 ...
 * site's translation run then ships a single language and nothing anywhere
 * says so -- an absence that reads as the reassuring answer, which is the most
 * expensive shape a check can have.
 *
 * So the request declares whether the site is multilingual and which
 * languages the run covers; this class reports what the site actually has; and
 * a disagreement REFUSES. The refusal names which side disagreed: WPML may
 * have been removed from the site, or the declaration may simply be wrong --
 * and those need different fixes.
 *
 * One plugin serves both kinds of site. There is no monolingual build: with
...
final class CadenceLanguageDeclaration {
    ...
    public static function verify($declared, string $language): array {
        ...
        if ($declared['multilingual'] && !$site_is_multilingual) {
            return self::no('capability_mismatch',
                'the request declares this site multilingual; this site implements no WPML '
                . 'translation-group hooks. Either WPML was removed or disabled here, or the record is wrong',
                $languages);
        }
        if (!$declared['multilingual'] && $site_is_multilingual) {
            return self::no('capability_mismatch',
                'the request declares this site monolingual; this site implements the WPML '
        ...
        if (!$declared['multilingual']) {
            // A monolingual declaration naming two languages is not a partial
     ...
     * stored as `en` under a `201` saying `placed: ["de"]`.
     *
