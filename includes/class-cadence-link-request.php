 ...
 *
 * TESTED AGAINST WORDPRESS 7.1 AND WPML 5.0.1. A falsy `trid` on
 * `wpml_set_element_language_details` creates a new trid for that element and
 * drops its existing relations (quoted above), and it does so PER ELEMENT --
 * the action is told about one element and has no way to know the call is one
 * of a set, which is why the ordered write below is necessary.
 * `wpml_element_language_details` read immediately afterwards, in the SAME
 * request, answers with the trid the write just invented, which is what lets
 * the create path complete live rather than across two requests.
 *
 * WPML reports only PUBLISHED posts from `wpml_get_element_translations`
 * unless it is asked with `all_statuses`, outside an admin request. See
 * `members_outside_plan`.
 *
...
final class CadenceLinkRequest {
    ...
    public static function run(array $plan, ?array $post_types, ?string $key_id = null,
        ...
        //
        // WITHOUT attestation, a holder of a leaked `translation.link` key
        // could associate posts with nothing proving the request came from
        // the tenant who holds the signing key -- and `source_language_code`
        // goes straight into WPML's write, so a tamper here ALTERS A WRITE
        // rather than merely causing a refusal.
        //
        ...
        // as agreeable and every write goes nowhere: a `200 {"written": 2}`
        // that linked nothing at all.
        //
        ...
        // than a report filed under an identity that has nothing to join it
        // to. `piece_id` and not a second spelling -- the wire carries
        ...
        // entitled to both.
        //
        // WHAT STAYS WIDE, deliberately: a
        ...
        // `LinkRequestTest::test_a_second_key_can_link_posts_published_before_stamping`
        // pins that behaviour.
        foreach ($posts as $p) {
        ...
        foreach ($posts as $p) {
            ...
            // Refusing on that condition would make the create path
            // unreachable and refuse the publish-then-link workflow at its
            // first call.
            //
        ...
        if ($create) {
            ...
            if ($group === null) {
                ...
                // longer requires every post to be in no group. The check
                // above admits a group whose every member this plan names, so
                // the source may have shared its group with the translations,
                // and the null-trid write has just moved it out of that group
                // and left them in it. Nothing OUTSIDE the plan can have been
                // detached, which is what the check guarantees, and a caller
                // that re-reads and sends the identical plan again heals it.
     ...
     * cannot see is worse than either outcome: the caller's own record would
     * file nothing for a run that changed the site. `written` is 1 by
     * construction and not a count of the loop: the loop below the refusal
     * never ran.
     *
     ...
     * detached" and writes. Observed behaviour, one group holding two drafts,
     * WPML 5.0.1 inside a REST request:
     *
     *   3 args                        -> []
     *   skip_missing false, all true  -> {"de":"10","en":"11"}
     *   skip_missing true,  all true  -> {"de":"10","en":"11"}
     *
     * The same three-argument call answers CORRECTLY under wp-cli, which is
     * not a REST request. It is also why the stub models the status filter:
     * without it, a revert to three arguments passes the whole suite and
     * silently detaches drafts on a real site.
     *
