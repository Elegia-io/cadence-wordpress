...
final class AdminTest extends TestCase {
    ...
    /**
     * THE ATTESTATION CELL KEEPS ITS MARKUP through the output-site `wp_kses`:
     * each kid in `<code>`, one per line, and the no-key warning in
     * `<em>`. An allow-list narrower than what the cell emits fails here.
     */
    public function test_the_attestation_cell_keeps_its_markup_through_the_allow_list(): void {
        $bare = CadenceKey::issue('tenant-a', ['content.publish'], 7);
        $held = CadenceKey::issue('tenant-b', ['content.publish'], 7);
        foreach (['abcdef0123456789', '0123456789abcdef'] as $kid) {
            $this->post(['do' => 'attest', 'id' => $held['id'], 'kid' => $kid,
                         'public_key' => base64_encode(random_bytes(32))]);
        }
        $html = $this->render();

        $this->assertStringContainsString(
            '<code>abcdef0123456789</code><br><code>0123456789abcdef</code>', $html);
        $this->assertStringContainsString('<em>none', $html);
    }
