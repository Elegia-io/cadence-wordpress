...
/**
 * Creating a post from content the caller produced.
 *
 * THE FAILURE THIS IS SHAPED AROUND IS THE DUPLICATE. An HTTP caller retries;
...
final class ContentRequestTest extends TestCase {
     ...
     * that skipped those would stop finding every existing post on the site and
     ...
     * a key is worth what its widest grant is worth: a tenant's caller that
     ...
     * compatibility path for every key already issued on the site.
     */
    #[Group('wpml')]
     ...
     * key entitled to neither. `post_type_exists` is a read of the site
     * itself and it happens only after the key's own scope has admitted the type.
     */
     ...
     * on the caller's next run -- content resurrected by a retry, which
     ...
     *
     * `placed` computed as `[$fields['language']]`, echoing the request
     * handed back, would agree with the caller by construction. The stub's
    ...
    public function test_placed_is_what_the_site_holds_and_not_what_was_asked(): void {
        ...
        // WPML files it under the site's default instead.
     ...
     * multilingual site that did not answer, which used to be reported as a
     * success.
     */
    #[Group('wpml')]
     ...
     *
     * Without this the omission surfaces on a live site instead of here.
     */
    #[Group('wpml')]
     ...
     * recording the piece's language. The predicate itself is that the
