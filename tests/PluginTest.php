...
final class PluginTest extends TestCase {
    ...
    public function test_the_route_takes_only_an_adopting_key(): void {
        ...
        foreach (['/adopt', '/adopt/preview'] as $path) {
    ...
    public function test_the_adopt_route_refuses_a_body_it_cannot_read(): void {
        foreach (['/adopt', '/adopt/preview'] as $path) {
            ...
            $this->assertSame('bad_adoption', $r->get_data()['code'], $path);
        }
    }
     ...
     * have hand-edited since. A caller that needs both holds both.
     */
     ...
     * caller that only ever publishes should not also be able to rearrange
     ...
     * GPL is what the header promises and what the file has to deliver, so the
     * two disagreeing is not cosmetic.
     */
     ...
     * is supposed to permit still happens, and this is the shape the caller
    ...
    public function test_publishing_two_languages_then_linking_them_still_succeeds(): void {
        ...
        // piece's language with WPML, so the write log holds those two
        ...
        // refuse, and this test is the one that said it did not.
     ...
     * registered routes.
     *
     * The shape this guards against is two brands sharing one WordPress, or an
     * agency site serving two separate tenants. Nothing writes
     ...
     * callers actually run -- which is what makes this a boundary rather than
     * a field.
     */
    #[Group('wpml')]
    public function test_one_tenants_key_cannot_link_another_tenants_posts(): void {
        ...
        // publishes above each recorded a language with WPML, and
