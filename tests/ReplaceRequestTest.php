 ...
 * article in wp-admin and fixed a sentence; the caller, holding a copy it
...
final class ReplaceRequestTest extends TestCase {
    ...
    /** Publish a piece the way the caller does, and hand back what it was told. */
    private function publish(array $over = [], ?string $key_id = CadenceAttest::KEY_ID,
        ...
        $r = CadenceContentRequest::run($body,
            ...
            // named key gets one the way the caller makes it rather than by
     ...
     * written asking for it before it was settled that the wire
     ...
     * piece already on the site was created before the stamp existed
    ...
    public function test_a_piece_that_predates_the_key_stamp_is_rewritten_by_any_key(): void {
        $published = $this->publish();
        // A POST FROM BEFORE THE STAMP EXISTED, made the way one on the
